...
taille_echantillon = 32
state_temp = [0,0,0,0,0,0]
alpha = 1 ##to be changed
gamma = 0.9
...
def train_network(batch,taille_batch,taille_entree,N):
	...
	with tf.name_scope("losses"): #fonction de perte
		l = tf.losses.mean_squared_error((1-alpha)*y_pred[np.argmax(x[1])], alpha*y[np.argmax(x[1])])## je ne propage que sur l'action qui a été faite.
	with tf.name_scope("optim"): #fonction d'optimisation
		train_op = tf.train.AdamOptimizer(1e-3).minimize(l)
	...
	with tf.Session() as sess:
		...
		while (True):
			...
			if (etape%taille_echantillon == 0 and len(batch)>taille_echantillon):## toutes les 100 étapes, on prend un échantillon du batch, et on rétro-propage le gradient.
				...
				loss_moyenne = 0 
				for k in range(taille_echantillon): ## on entraîne plusieurs fois?
					##print("reward : ",reward)
					echantillon = random.sample(batch,taille_echantillon)
					x_dict = []
					y_dict = []
					## on change les poids, ie on met à jour le réseau secondaire.
					for i in range(len(echantillon)):## il y a déjà l'état courant.
						##print("i : reward : ",i,reward)
						x_dict.append(echantillon[i][0]);## mon état actuel
						max_Q = max(Q_valeur(W1f, b1f, W2f, b2f, W3f, b3f, Wof, bof,echantillon[i][3]))## le max des Q-valeur de l'état
						## après transition
						max_Q = [max_Q,max_Q]
					...
					W1f, b1f, W2f, b2f, W3f, b3f, Wof, bof = sess.run([W1, b1, W2, b2, W3, b3, Wo, bo])
					print("loss : ",loss)
				print("loss moyenne: ",loss_moyenne/100);
			...
			if (random.random() > coeff_explo): ## exploration
				...
				if (random.random()>0.5): ##soit aller en haut, soit ne rien faire, au hasard.
					...
					action = [1,0]
			image,reward,terminal,state  = game_state.frame_step(action) ## /!\ On récupère l'état - et le reward - qui suit l'action "action" !
			##print("reward : ",reward)
			if (len(batch)>=20000):
			...
			print("avec les paramètres : ","y :",state[0],"velocity y :",state[1]);
			print("x du tuyau en haut : ",state[2],"y du tuyau en haut : ",state[3]);
			print("y du tuyau en bas",state[4]);
