  ...
  with tf.Session() as sess:
    ...
    while (True):
    	...
    	if (etape%taille_echantillon == 0 and len(batch)>taille_echantillon):## toutes les 100 étapes, on prend un échantillon du batch, et on rétro-propage le gradient.
    		...
    		for k in range(1000): ## on entraîne plusieurs fois?
    			##print("reward : ",reward)
	    		echantillon = random.sample(batch,taille_echantillon)
	    		W1f, b1f, W2f, b2f, W3f, b3f, Wof, bof = sess.run([W1, b1, W2, b2, W3, b3, Wo, bo])
	    		x_dict = [state]## celui d'en ce-moment
	    		y_dict = [np.asarray(alpha*(np.asarray(Q_valeur(W1f, b1f, W2f, b2f, W3f, b3f, Wof, bof,state,reward))))*gamma + reward]## celui d'en ce-moment
	    		for i in range(len(echantillon)-1):## il y a déjà l'état courant.
	    			##print("i : reward : ",i,reward)
	    			x_dict.append(echantillon[i][0]);
	    			y_dict.append(np.asarray(alpha * (np.asarray(Q_valeur(W1f, b1f, W2f, b2f, W3f, b3f, Wof, bof,echantillon[i][3],echantillon[i][2]))))+echantillon[i][2]);
    	...
    	prochaine_action = Q_valeur(W1f, b1f, W2f, b2f, W3f, b3f, Wof, bof,state,reward);
    	print("le réseau veut sauter à : ",prochaine_action[0],"et ne pas sauter à ",prochaine_action[1]);
  
  

def Q_valeur(W1f,b1f,W2f,b2f,W3f,b3f,Wof,bof,state,reward) : ##le réseau secondaire, pour l'instant. Rq.: il est constant sur tout une série de mini-batch (100 étapes).
