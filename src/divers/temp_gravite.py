...
player = pygame.image.load('images/flap.gif').convert()
background = pygame.image.load('images/fond2.png').convert()
tuyau_bas = pygame.image.load('images/tuy_bas.png').convert()
tuyau_haut = pygame.image.load('images/tuy_haut.png').convert()
...
trajY = []
for i in range(100):
	trajY.append(+g*((i/1000)**2) - v0*(i/1000))
count = 0
class GameObject:
		speed = 0
		image = ""
		...
		def move(self):
			...
			if self.pos[1] >400 or self.pos[1] <0:
				self.pos[1] = 0
			self.traj += 1
			return -1
