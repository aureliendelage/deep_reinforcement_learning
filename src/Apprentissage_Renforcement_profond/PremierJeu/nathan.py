	...
	if (x==4 and y==4):
		...
		try:
			if (Marqued[x+1][y] < 1):
				...
				print("marqued :",Marqued[x+1][y])
				print("board : ",Board[x+1][y])
				print("Fonction_probabilite : ",Fonction_probabilite[x][y][1])
				print ("a vaut : ",a)
				Marqued[x+1][y] = 1
			else:
				if (Board[x+1][y] != 0):
					print("je multiplie par : ",Fonction_probabilite[x][y][1])
					a = Fonction_probabilite[x][y][1]*Board[x+1][y]
				else:
					a = Board[x+1][y]*Fonction_probabilite[x][y][1]+Fonction_Valeur(x+1,y,n+1,Board,Marqued)
					print ("a vautt : ",a)
					Marqued[x+1][y] = 1
		...
		try:
			if (Marqued[x][y+1] ==0):
				...
				if (Board[x][y+1] != 0):
					print("je multiplie par : ",Fonction_probabilite[x][y][0])
					b = Fonction_probabilite[x][y][0]*Board[x][y+1]
				else:
					b = Board[x][y+1]*Fonction_probabilite[x][y][0]+Fonction_Valeur(x,y+1,n+1,Board,Marqued)
					print ("b vautt : ",b)
					Marqued[x][y+1] = 1
		...
		try:
			if (Marqued[x-1][y] ==0):
				...
				if (Board[x-1][y] != 0):
					print("je multiplie par : ",Fonction_probabilite[x][y][3])
					c = Fonction_probabilite[x][y][3]*Board[x-1][y]
				else:
					c = Board[x-1][y]*Fonction_probabilite[x][y][3]+Fonction_Valeur(x-1,y,n+1,Board,Marqued)
					print ("c vautt : ",c)
					Marqued[x-1][y] = 1
		...
		try:
			if (Marqued[x][y-1] ==0):
				...
				if (Board[x][y-1] != 0):
					print("je multiplie par : ",Fonction_probabilite[x][y][2])
					d = Fonction_probabilite[x][y][2]*Board[x][y-1]
				else:
					d = Board[x][y-1]*Fonction_probabilite[x][y][2]+Fonction_Valeur(x,y-1,n+1,Board,Marqued)
					print ("d vautt : ",d)
					Marqued[x][y-1] = 1
		...
		print("au point : ",x," ",y,"a : ",a,"b : ",b," c :",c," d : ",d)
