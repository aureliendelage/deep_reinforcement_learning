...
with tf.name_scope("layer_2"):
  ...
  W1 = tf.Variable(tf.random_normal([hidden, hidden_2]))  ## 
  b1 = tf.Variable(tf.zeros([hidden])) ## biais pour les hidden neurones cachés
  layer_2 = tf.sigmoid(tf.matmul(layer_1,W1) + b1)
with tf.name_scope("layer_3"):
  W2 = tf.Variable(tf.random_normal([hidden_2, hidden_3]))  ## 
  b2 = tf.Variable(tf.zeros([hidden_2])) ## biais pour les hidden neurones cachés
  layer_3 = tf.sigmoid(tf.matmul(layer_2,W2) + b2)
